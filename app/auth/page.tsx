...
export default function AuthPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    // Verify the account role matches what they selected
  ...
  async function handleStudentSignUp(e: React.FormEvent) {
    ...
    setPendingEmail(email); setPendingName(firstName)
        ...
        {panel === 'success' && (
          <div className={`auth-card p-8 md:p-10 text-center ${anim}`}>
            ...
            <p className="text-sm font-body text-muted mb-8">
              {isLandlord
                ? 'Your landlord account is verified. Start listing your properties.'
                : 'Your .edu is verified. You\'re officially in.'}
            </p>
            <a href={isLandlord ? '/landlord' : '/'}
              ...
              {isLandlord ? 'Go to My Dashboard →' : 'Browse Listings →'}
            </a>
          </div>
        )}
